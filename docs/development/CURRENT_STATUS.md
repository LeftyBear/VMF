...
| VMF Build P5-09 GenerateContext Focused Test Implementation Start | STARTED as local-only implementation scope, not docs-only. `docs/spec/P5-09_GenerateContextFocusedTestImplementationStart.md` records the P5-09 start boundary from P5-08, requires exact GenerateContext entry boundary and focused test target identification before code or test edits, and preserves the P5-04 through P5-08 hard stops before Generator. Fallback, implicit Template selection, Template content inference, GenerateContext / Generator compensation, Template file changes, Generator invocation or behavior change, runtime generation behavior change, package / `dist` / release operation, external service operation, public API change, persisted schema change, canonical format change, and Frozen specification change remain out of scope. |

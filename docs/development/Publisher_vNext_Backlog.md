...
- P2-28: candidate selection after P2-27; docs-only / local-only selection
  complete. It compares remaining P2 routes, selects P2-26-derived
  `preview-update` implementation-scope planning as the next bounded
  candidate, and fixes scope / non-scope / verification boundaries. It does
  not implement `preview-update`, adopt Google Picker or `drive.file`, change
  OAuth scopes, call Google Docs / Drive, inspect or mutate token stores, run

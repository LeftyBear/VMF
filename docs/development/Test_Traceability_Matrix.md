...
| Release versioning, tag, artifact identity, evidence identity, and approval/authorization identity | `Publisher_ReleaseApprovalPackage.md`; `Publisher_AvastResponseIntakeTemplate.md`; `CURRENT_STATUS.md`; `docs/releases/Publisher_0.0.1-dev_ReleaseNotes.md` | ADR-0016 | Release identity fields recorded for `0.0.1-dev`; package tooling exists and the fixed package identity is recorded | N/A | N/A | Version, target commit, intended tag, package path, package size, SHA-256, package generation PASS, package verification PASS, final verification PASS, Live E2E 4/4 PASS, and result review complete are recorded; tag creation, GitHub Release, publication, approval record, and authorization record remain pending | Current status and release notes record the fixed package identity and that this docs update did not write to `dist` or authorize tag/release execution | Partial: package identity recorded; tag/release authorization and publication blocked |
...
| Release readiness | Pending tag/release sequence after final verification, Live E2E, result review, package generation, and package verification are recorded as PASS/complete |
| Release gate | Hold lifted by ADR-0019 VMF risk acceptance; release execution advanced through package verification only |
...
| Release, tag, publication artifacts | Not created by this task; intended tag `vmf-publisher-v0.0.1-dev` has not been created |

...
| Evidence bundle specification | Completed: `docs/development/Publisher_EvidenceBundleSpecification.md` | Defines the redacted evidence index and review structure; it does not assemble a concrete bundle, approve release readiness, or create vendor-clearance evidence. |
| Avast response intake template | Template only: `docs/development/Publisher_AvastResponseIntakeTemplate.md` | Records the safe intake route for a future vendor response; no Avast response has been received, vendor clearance remains not obtained, and Avast safety certification is not claimed. |
